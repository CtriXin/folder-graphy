#!/usr/bin/env node
import { findCallers, formatResult } from "../query/engine.js";
import { ensureMapIndex, parseQueryCliOptions, resolveQueryProject, toQueryOptions } from "./shared.js";

function usage(): void {
  console.error("Usage: map-callers <symbol> [--cwd <dir>] [--scope <path>] [--changed] [--from <ref>] [--to <ref>] [--json]");
}
...
async function main(): Promise<void> {
  ...
  const results = findCallers(symbol, toQueryOptions(cliOptions, projectPath));
  if (cliOptions.json) {
