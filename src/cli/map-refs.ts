...
function usage(): void {
  console.error("Usage: map-refs <symbol> [--cwd <dir>] [--scope <path>] [--changed] [--from <ref>] [--to <ref>] [--json]");
}
...
async function main(): Promise<void> {
  ...
  const results = findReferences(symbol, toQueryOptions(cliOptions, projectPath));
  if (cliOptions.json) {
